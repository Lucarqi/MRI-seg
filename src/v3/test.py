...
parser.add_argument('--generator_B2A', type=str, default='output/cyclegan/4/netG_B2A.pth', help='B2A generator checkpoint file')
parser.add_argument('--trans_name',type=str, default='cyclegan',help='trans type of dataset')
parser.add_argument('--histogram_match', type=bool, default=False, help='need histogram match in data preporcess or not')
...
dataloader = DataLoader(ImageDataset(transforms_=transforms_['valid'],opt=opt, mode='valid'), 
...
for i, batch in enumerate(dataloader):
    ...
    # save as .nii.gz
    img = fake_B.detach().cpu().numpy()
    img = np.squeeze(img,axis=0) # [1,256,256]
    save = sitk.GetImageFromArray(img)
    sitk.WriteImage(save,'datasets/train/fake_lge/%s.nii.gz'%(info))
